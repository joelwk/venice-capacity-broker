...
def build_store() -> TenantStore:
	...
	if backend == "sql" and SQLTenantStore is not None:
		try:
			...
			if prod:
				logger.critical("broker.store: SQL backend required in production; init failed: %s", e)
				raise
			logger.warning(
				"broker.store: SQL backend requested but failed: %s", e
			)
			# fall through to JSON gating below
	else:
		if backend == "sql":
			if prod:
				logger.critical("broker.store: SQL backend required in production; SQLTenantStore unavailable")
				raise RuntimeError("SQL tenant store unavailable in production")
			logger.warning("broker.store: SQL backend requested but SQLTenantStore unavailable; will consider JSON fallback")

	# JSON fallback gating
	if prod:
		logger.critical("broker.store: JSON backend is forbidden in production; set BROKER_STORE_BACKEND=sql and configure Postgres")
		raise RuntimeError("Production requires SQL tenant store; JSON backend forbidden")

	# Non-production: allow JSON only with explicit flag
	if not env_flag("ALLOW_JSON_FALLBACK", False):
