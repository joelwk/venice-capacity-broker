...
try:
    from .routers import admin, bids, clearing, marketdata as marketdata_router, purchases, quotes, settlement, tenants, venice
except ImportError:
    ...
    from apps.broker_api.routers import admin, bids, clearing, marketdata as marketdata_router, purchases, quotes, settlement, tenants, venice
...
try:
    ...
    from .services import bids as bids_helpers
    from .services import clearing as clearing_helpers
    from .services import pricing as pricing_helpers
except ImportError:
    # Fallback for direct module loading (e.g., in tests)
    import sys
    from pathlib import Path
    ...
    from apps.broker_api.services import bids as bids_helpers
    from apps.broker_api.services import clearing as clearing_helpers
    from apps.broker_api.services import pricing as pricing_helpers
...
_SHARED_STATE = sys.modules.setdefault("apps.broker_api._shared_state", types.SimpleNamespace())
...
def _setup_static_routes(app: FastAPI) -> None:
    ...
    try:
        _buy_html_path = (Path(__file__).resolve().parent.parent / "control-plane" / "buy.html").resolve()
...
def _setup_cors(app: FastAPI) -> None:
    ...
    try:
        if (os.getenv("CORS_ENABLED") or "false").strip().lower() in {"1", "true", "yes", "on"}:
...
def _build_env_status() -> dict[str, Any]:
    ...
    quotes_enabled = (os.getenv("QUOTES_ENABLED") or "true").strip().lower() in {"1", "true", "yes", "on"}
    purchases_enabled = (os.getenv("PURCHASES_ENABLED") or "true").strip().lower() in {"1", "true", "yes", "on"}
    clearing_enabled = (os.getenv("CLEARING_ENABLED") or "false").strip().lower() in {"1", "true", "yes", "on"}
    bids_enabled = (os.getenv("BIDS_ENABLED") or "false").strip().lower() in {"1", "true", "yes", "on"}
...
def _build_dependencies() -> dict[str, Any]:
    ...
    # Core services - build fresh
    deps["store"] = store.build_store()
    ...
    limiter, kv_admin, enabled, window_seconds, max_requests = rate_limit.build_rate_limiter()
    ...
    try:
        deps["pricing_service"] = pricing_helpers.build_pricing_service()
        deps["settle_pricing"] = pricing_helpers.build_pricing_service()
    except Exception:
    ...
    if deps["has_sqlmodel"]:
        try:
            from db.models import Quote, Purchase, Counter, Bid
    ...
    if bids_enabled:
        ...
        deps["price_usdc_per_unit_from_asset"] = lambda max_price_minor, asset: bids_helpers.price_usdc_per_unit_from_asset(
            max_price_minor, asset, marketdata.get_marketdata_provider
        )
        deps["classify_bid_status"] = lambda max_price_usdc, now_s, expiry_s: bids_helpers.classify_bid_status(
            max_price_usdc, now_s, expiry_s, 
            lambda: clearing_helpers.compute_clearing_price(marketdata.get_marketdata_provider())
        )
    ...
    if clearing_enabled:
        deps["compute_clearing_price"] = lambda: clearing_helpers.compute_clearing_price(
            marketdata.get_marketdata_provider()
        )
    
    # Settlement configuration
    settlement_enabled = (os.getenv("SETTLEMENT_ENABLED") or "false").strip().lower() in {"1", "true", "yes", "on"}
...
def _wire_routers(app: FastAPI, deps: dict[str, Any]) -> None:
    ...
    try:
        ...
        def _provider_proxy():
            ...
            if callable(candidate):
                if os.getenv("BROKER_DEBUG_PROVIDER_PROXY") == "1":
                    print(f"[provider_proxy] globals candidate from {getattr(candidate, '__module__', '')}", flush=True)
            ...
            if isinstance(module_refs, list):
                for mod in reversed(module_refs):
                    ...
                    if os.getenv("BROKER_DEBUG_PROVIDER_PROXY") == "1":
                        print(f"[provider_proxy] module_ref candidate from {candidate_module}", flush=True)
                    if candidate_module.startswith("tests.") or candidate_module == "__main__":
                        provider_factory = candidate
                        break
            ...
            if os.getenv("PYTEST_CURRENT_TEST"):
                for name, test_mod in list(sys.modules.items()):
                    ...
                    if callable(candidate) and getattr(candidate, "__module__", "").startswith("tests."):
                        provider_factory = candidate
                        break
            ...
            if not callable(provider_factory):
                ...
                if not callable(candidate):
                    ...
                    if app_module is not None and hasattr(app_module, "_get_marketdata_provider"):
                        candidate = getattr(app_module, "_get_marketdata_provider")
                if callable(candidate):
                    candidate_module = getattr(candidate, "__module__", "")
                    if (
            ...
            if os.getenv("BROKER_DEBUG_PROVIDER_PROXY") == "1":
                try:
                    identifier = getattr(provider_factory, "__name__", repr(provider_factory))
                except Exception:
            ...
            if (
                ...
                and getattr(provider_factory, "__module__", __name__).startswith("apps.")
            ):
                try:
                    import gc
                    ...
                    for obj in gc.get_objects():
                        ...
                        for cell in closure:
                            try:
                                ...
                                continue
                            if hasattr(contents, "calls") and isinstance(getattr(contents, "calls", None), list):
                                if os.getenv("BROKER_DEBUG_PROVIDER_PROXY") == "1":
                                    print(f"[provider_proxy] gc candidate from {module_name}", flush=True)
        ...
        def _env_status_proxy():
            ...
            app_module = sys.modules.get("apps.broker_api.app")
            if app_module is not None and callable(getattr(app_module, "env_status", None)):
                status_fn = getattr(app_module, "env_status")
...
def __setattr__(name: str, value: object) -> None:  # type: ignore[override]
    ...
    if name == "_get_marketdata_provider" and callable(value):
        ...
        if canonical is not None and canonical is not current:
            setattr(canonical, "_provider_override", value)
