...
def _is_explicit_false(raw: Optional[str]) -> bool:
...
def current_admin_token() -> Optional[str]:
...
def validate_admin_config() -> None:
    ...
    if require_admin and not token:
        logger.error(
            "security: BROKER_REQUIRE_ADMIN_TOKEN=true but BROKER_ADMIN_TOKEN is unset; refusing to start"
        )
    ...
    if token:
        ...
        logger.warning(
            "security: BROKER_ADMIN_TOKEN not set; admin endpoints allowed for development only"
        )
...
def require_admin(authorization: Optional[str]) -> None:
    ...
    """
    # Read env vars dynamically to support test scenarios that set them after module load
    require_admin, explicitly_false = _resolve_admin_requirement()
    admin_token = current_admin_token()
    
    # If REQUIRE_ADMIN is explicitly set to False, skip auth check (for development/testing)
...
def tenant_by_subkey(store: TenantStore, token: Optional[str]) -> Optional[Tenant]:
...
def auth_context(store: TenantStore, authorization: Optional[str]) -> tuple[str, Optional[Tenant]]:
