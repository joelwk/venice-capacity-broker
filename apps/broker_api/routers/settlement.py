...
def settlement_preview(
    ...
    amountOut: int = Query(
        ..., description="Desired output amount in minor units (wei or 6dp)"
    ),
    path: str | None = Query(
        None, description="Optional CSV path override: addr0,addr1,[addr2]"
    ),
) -> dict:
    ...
    if asset_u == "ETH":
        to_token = (
            __os.getenv("WETH_ADDRESS") or "0x4200000000000000000000000000000000000006"
        ).strip()
    else:
        to_token = (
            __os.getenv("USDC_ADDRESS")
            or __os.getenv("QUOTE_TOKEN_ADDRESS")
            or "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"
        ).strip()
    if not to_token:
        raise HTTPException(
            status_code=400, detail="target token address not configured"
        )
    ...
    risk_violation_detail: str | None = None
    ...
    if q is not None:
        ...
        try:
            ...
            def _normalized_exec_price(
                amount_in: int, dec_in: int, amount_out: int, dec_out: int
            ) -> float | None:
                try:
            ...
            exec_price_norm = _normalized_exec_price(
                int(q.amount_in), di, int(q.amount_out), do
            )
            ref_price_norm = None
            try:
                ...
                try:
                    ...
                    simple_slip_bps = int(
                        round(
                            max(
                                0.0,
                                (float(ref_price_norm) - float(exec_simple))
                                / float(ref_price_norm)
                                * 10000.0,
                            )
                        )
                    )
                    _logger.debug(
                        "exec_simple=%s ref=%s simple_slip=%s",
                        exec_simple,
                        ref_price_norm,
                        simple_slip_bps,
                    )
                    slip_candidates.append(
                        policy.check_slippage(exec_simple, ref_price_norm)
                    )
                except Exception:
                    pass
            ...
            if input_reserve and output_reserve and float(q.amount_in) > 0:
                ...
                pool_take_bps_val = int(
                    round((float(q.amount_in) / float(input_reserve)) * 10000)
                )
            if exec_price_norm is not None and ref_price_norm and ref_price_norm > 0:
                slip_candidates.append(
                    policy.check_slippage(exec_price_norm, ref_price_norm)
                )
            if exec_price_raw is not None and ref_price_raw and ref_price_raw > 0:
                slip_candidates.append(
                    policy.check_slippage(exec_price_raw, ref_price_raw)
                )
            max_slippage_bps = int(policy.slippage_bps_cap)
            if simple_slip_bps is not None:
                _logger.debug(
                    "simple_slip=%s cap=%s", simple_slip_bps, max_slippage_bps
                )
            ...
            if slip_candidates:
                worst = max(
                    slip_candidates,
                    key=lambda r: float(r.get("slippage_bps", 0.0) or 0.0),
                )
    ...
    try:
        ...
        if input_reserve and output_reserve and output_reserve > 0:
            amt_in_units_raw = int(
                math.ceil(
                    (float(amt_out) * float(input_reserve)) / float(output_reserve)
                )
            )
            if amt_in_units_raw > 0:
                amt_in_units = amt_in_units_raw
            pool_take_bps_val = int(
                round((float(amt_in_units) / float(input_reserve)) * 10000)
            )
        ...
        if input_reserve and output_reserve and float(amt_in_units) > 0:
            ...
            ref_price_raw = (
                float(output_reserve) / float(input_reserve)
                if float(input_reserve) > 0
                else None
            )
        ...
        if slip_candidates:
            worst = max(
                slip_candidates, key=lambda r: float(r.get("slippage_bps", 0.0) or 0.0)
            )
            slip_bps_val = int(round(float(worst.get("slippage_bps", 0.0) or 0.0)))
            max_slippage_bps = int(policy.slippage_bps_cap)
            if slip_bps_val > max_slippage_bps or not bool(worst.get("ok", True)):
                raise HTTPException(
                    status_code=409,
                    detail=f"estimated slippage {slip_bps_val} bps exceeds cap of {max_slippage_bps} bps",
                )
        ...
        if cap_units is not None and int(amt_in_units) > int(cap_units):
            ...
            if pool_take_bps_val is not None:
                raise HTTPException(
                    status_code=409,
                    detail=f"input exceeds pool take cap: {pool_take_bps_val} bps > {max_pool_take_bps} bps allowed",
                )
