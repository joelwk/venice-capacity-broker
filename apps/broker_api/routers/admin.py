            ...
            out.append(
                {
                    ...
                    "status": r.status,
                    "expiresAt": (
                        r.expires_at.strftime("%Y-%m-%dT%H:%M:%SZ")
                        if r.expires_at
                        else None
                    ),
                    "createdAt": (
            ...
            out.append(
                {
                    ...
                    "fulfilledAt": (
                        r.fulfilled_at.strftime("%Y-%m-%dT%H:%M:%SZ")
                        if r.fulfilled_at
                        else None
                    ),
                }
...
def admin_counters(
    ...
    scope: str | None = Query(
        default=None, description="Optional scope filter (e.g., 'chat', 'signals')"
    ),
    bucket_seconds: str | None = Query(
        default=None, description="Optional bucket size filter"
    ),
    ...
    try:
        bucket_seconds_value = (
            None if bucket_seconds is None else int(str(bucket_seconds).strip())
        )
...
def venice_probe(
    base: str | None = Query(
        default=None, description="Base host, e.g., https://api.venice.ai"
    ),
...
__all__ = ["debug_router", "init_router", "router"]
