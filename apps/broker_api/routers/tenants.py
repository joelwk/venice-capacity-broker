...
def _get_parent_venice_key() -> str:
    key = os.getenv("VENICE_PARENT_KEY") or os.getenv("VENICE_API_KEY")
    if not key:
        raise HTTPException(
            status_code=500, detail="VENICE_PARENT_KEY or VENICE_API_KEY must be set"
        )
    return key
...
_auth_context: Callable[[str | None], tuple[str, Tenant | None]]
_compute_expires_at: Callable[[float | None], str | None]
...
_limiter: Any
_get_rate_limit_headers: Callable[[str | None], dict[str, str]]
...
def init_router(
    *,
    ...
    auth_context: Callable[[str | None], tuple[str, Tenant | None]],
    compute_expires_at: Callable[[float | None], str | None],
    ...
    limiter: Any,
    get_rate_limit_headers: Callable[[str | None], dict[str, str]],
) -> APIRouter:
...
async def create_tenant(
    ...
    rotate: bool = Query(
        default=False,
        description="If true and tenant exists, mint a new subkey and update store",
    ),
    revoke_old: bool = Query(
        default=False,
        description="When rotating and an old key_id exists, revoke it after successful rotate",
    ),
) -> TenantResponse:
    _require_admin(authorization)
    ...
    if not parent_key:
        raise HTTPException(
            status_code=400, detail="VENICE_PARENT_KEY or VENICE_API_KEY must be set"
        )
    ...
    if existing_t and rotate:
        ...
        expires_at = (
            req.expires_at or existing_t.expires_at or _compute_expires_at(None)
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to create subkey: {exc}"
        ) from exc
    ...
    if not subkey:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Subkey not returned by Venice (fields={keys_present})",
        )
    ...
    if rotate and revoke_old and old_key_id:
        try:
            ...
            _logger.warning(
                "rotate: revoke_old failed for key_id=%s: %s", old_key_id, exc
            )
    return TenantResponse(
        id=tenant.id,
        label=tenant.label,
...
def get_tenant(
    ...
    if not t:
        raise HTTPException(status_code=404, detail="tenant not found")
    return TenantResponse(
        id=t.id, label=t.label, quota=t.quota, expires_at=t.expires_at, status=t.status
    )
...
def set_broker_limits(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"failed to update limits: {exc}"
        ) from exc
...
async def chat_proxy(
    ...
    if role == "tenant":
        ...
        if not x_tenant_id:
            raise HTTPException(
                status_code=400, detail="X-Tenant-Id required for admin calls"
            )
    ...
    _def_model = (
        os.getenv("BROKER_DEFAULT_MODEL") or os.getenv("VENICE_DEFAULT_MODEL") or ""
    ).strip()
    ...
    # Use the parent Venice key as the Authorization bearer and attach the
    # per-tenant subkey via X-Venice-Sub-Key. This matches Venice's preferred
    # pattern for scoped keys and avoids 401s when subkeys are not valid as
    # standalone bearer tokens.
    parent_key = _get_parent_venice_key()
    sub_client = VeniceClient(api_key=parent_key, base_url=_client.config.base_url)
    ...
    try:
        result = None
        if _model:
            result = sub_client.chat_completions(
                messages=payload.messages,
                model=_model,
                sub_key=t.subkey,
                **extra_args,
        ...
        # Propagate FastAPI HTTPException responses unchanged.
        raise
    except RuntimeError as exc:
        # Venice client wraps non-2xx responses in RuntimeError with a hint string.
        # Preserve upstream 429s so DIEM quota exhaustion is visible to tenants,
        # and expose all other failures as 5xx broker errors.
        message = str(exc)
        if "Venice error 429" in message:
            raise HTTPException(status_code=429, detail=message) from exc
        raise HTTPException(status_code=502, detail=f"venice error: {message}") from exc
    except Exception as exc:
...
def tenant_usage(
    ...
    if role == "tenant":
        ...
        if tenant_ctx.status != "active" or t.status != "active":
            raise HTTPException(status_code=401, detail="invalid tenant")
    ...
    sub_client = VeniceClient(api_key=parent_key, base_url=_client.config.base_url)
    try:
        usage = sub_client.get_usage(sub_key=t.subkey)
    except Exception as exc:
        raise HTTPException(
            status_code=502, detail=f"usage fetch failed: {exc}"
        ) from exc
    try:
        limits = sub_client.get_rate_limits(sub_key=t.subkey)
    except Exception as exc:
...
def tenant_limits(
    ...
    sub_client = VeniceClient(api_key=parent_key, base_url=_client.config.base_url)
    try:
        return sub_client.get_rate_limits(sub_key=t.subkey)
    except Exception as exc:
        raise HTTPException(
            status_code=502, detail=f"limits fetch failed: {exc}"
        ) from exc
...
def whoami(
    ...
    role, tenant_ctx = _auth_context(authorization)
    if role == "tenant":
        if tenant_ctx is None or tenant_ctx.status != "active":
            raise HTTPException(status_code=401, detail="invalid tenant")
    ...
    return {
        "role": "tenant",
        "tenant": {"id": tenant_ctx.id, "label": tenant_ctx.label},
    }
...
def my_usage(
    ...
    sub_client = VeniceClient(api_key=parent_key, base_url=_client.config.base_url)
    try:
        usage = sub_client.get_usage(sub_key=tenant_ctx.subkey)
    except Exception as exc:
        raise HTTPException(
            status_code=502, detail=f"usage fetch failed: {exc}"
        ) from exc
    try:
        limits = sub_client.get_rate_limits(sub_key=tenant_ctx.subkey)
    except Exception as exc:
        _logger.warning("limits fetch failed: %s", exc)
        limits = None
    return UsageResponse(usage=usage, limits=limits)
...
def my_set_broker_limits(
    ...
    if limits.windowSeconds is not None:
        try:
            ...
            raise HTTPException(
                status_code=422, detail="invalid windowSeconds"
            ) from exc
        if ws < int(current.get("windowSeconds", ws)):
            raise HTTPException(
                status_code=403, detail="cannot decrease windowSeconds (admin only)"
            )
    ...
    if limits.maxRequests is not None:
        ...
        if mr > int(current.get("maxRequests", mr)):
            raise HTTPException(
                status_code=403, detail="cannot increase maxRequests (admin only)"
            )
        current["maxRequests"] = mr
    if limits.label is not None:
        if not str(limits.label).startswith("self:"):
            raise HTTPException(
                status_code=403, detail="label must be prefixed with 'self:'"
            )
        current["label"] = limits.label
