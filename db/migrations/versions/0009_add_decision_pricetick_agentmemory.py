...
def upgrade() -> None:
    ...
    if not _has_table(conn, "decision"):
        ...
        if not _has_index(conn, "decision", "ix_decision_agent"):
            op.create_index(
                "ix_decision_agent", "decision", ["agent"]
            )  # idempotent in most backends
        if not _has_index(conn, "decision", "ix_decision_created_at"):
            op.create_index(
                "ix_decision_created_at", "decision", ["created_at"]
            )  # ditto
    ...
    if not _has_table(conn, "agentmemory"):
        op.create_table(
            ...
            sa.Column(
                "decision_id", sa.Integer(), sa.ForeignKey("decision.id"), nullable=True
            ),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        ...
        if not _has_index(conn, "agentmemory", "ix_agentmemory_agent"):
            op.create_index(
                "ix_agentmemory_agent", "agentmemory", ["agent"]
            )  # fast filter by agent
        if not _has_index(conn, "agentmemory", "ix_agentmemory_created_at"):
            op.create_index(
                "ix_agentmemory_created_at", "agentmemory", ["created_at"]
            )  # retention pruning
