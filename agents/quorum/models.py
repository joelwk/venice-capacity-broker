...
def _env_float(name: str, default: float | None = None) -> Optional[float]:
    raw = os.getenv(name)
...
class QuorumContext:
    ...
    price_guard: Optional[Dict[str, Any]] = None
...
class YieldModel(BaseModel):
    ...
    inactive_reasons_hold: set[str] = field(default_factory=lambda: {"inactive_staker", "heartbeat_failed"})
    ...
    def vote(self) -> QuorumVote:
        ...
        if status not in (None, "ok"):
            return QuorumVote(approve=False, confidence=0.9, reason=f"stake_status:{status}")

        active = bool(snapshot.get("active_staker")) if isinstance(snapshot, dict) else False
        heartbeat_err = (heartbeat or {}).get("error") if isinstance(heartbeat, dict) else None
        if not active:
            return QuorumVote(approve=False, confidence=0.8, reason="inactive-staker")
        if heartbeat_err:
            return QuorumVote(approve=False, confidence=0.6, reason=f"heartbeat:{heartbeat_err}")
        rewards = snapshot.get("unclaimed_rewards") if isinstance(snapshot, dict) else None
        has_rewards = bool(rewards) and float(rewards) > 0.0 if rewards is not None else False
        reason = "stake_healthy_with_rewards" if has_rewards else "stake_healthy"
        confidence = self._clamp_conf(self.min_conf_active + (0.1 if has_rewards else 0.0))
...
class ArbModel(BaseModel):
    ...
    def vote(self) -> QuorumVote:
        ...
        if not ctx.simulate_decision:
            return QuorumVote(approve=False, confidence=0.4, reason="no-simulated-decision")
        ...
        if premium >= self.premium_threshold:
            ...
            return QuorumVote(approve=True, confidence=conf, reason=f"premium:{premium:.3f}")
        return QuorumVote(approve=False, confidence=0.6, reason=f"premium:{premium:.3f}")
...
class RiskModel(BaseModel):
    ...
    max_vol_bps: Optional[float] = None
    halt_reasons_block: set[str] = field(default_factory=lambda: {"price_guard", "reflex_halt", "stake_error"})
    ...
    def vote(self) -> QuorumVote:
        ...
        if guard_status in self.halt_reasons_block:
            return QuorumVote(approve=False, confidence=0.9, reason=f"price-guard:{guard_status}")

        vol = ctx.vol_bps
        if vol is not None and self.max_vol_bps is not None and vol > self.max_vol_bps:
            return QuorumVote(approve=False, confidence=0.85, reason=f"vol-bps:{vol:.1f}")

        warnings = reflex.get("warnings")
        if isinstance(warnings, list) and warnings:
            return QuorumVote(approve=False, confidence=0.7, reason=f"reflex-warning:{warnings[0]}")
...
class DemandModel(BaseModel):
    ...
    def vote(self) -> QuorumVote:
        ...
        if util >= self.surge_threshold:
            intensity = (util - self.surge_threshold) / max(1e-6, 1.0 - self.surge_threshold)
            conf = self._clamp_conf(0.6 + intensity * 0.4, default=0.6)
            return QuorumVote(approve=True, confidence=conf, reason=f"util-hot:{util:.3f}")
        if util <= self.relax_threshold:
            ...
            return QuorumVote(approve=False, confidence=conf, reason=f"util-slack:{util:.3f}")
        return QuorumVote(approve=False, confidence=0.45, reason=f"util-neutral:{util:.3f}")
...
class TreasuryModel(BaseModel):
    ...
    def vote(self) -> QuorumVote:
        ...
        if avg_diem is None:
            return QuorumVote(approve=False, confidence=self.min_conf, reason="usage-unknown")
        try:
            avg = float(avg_diem)
        except (TypeError, ValueError):
            return QuorumVote(approve=False, confidence=self.min_conf, reason="usage-invalid")
        ...
        if delta <= 0:
            conf = self._clamp_conf(self.min_conf + min(0.2, abs(delta) / max(1.0, target)), default=self.min_conf)
