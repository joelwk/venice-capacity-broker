...
def test_rotate_revoke_old_key(monkeypatch, tmp_path):
    ...
    def fake_issue(parent_key, label, consumption_limit, expires_at):  # noqa: ANN001
    ...
    existing = Tenant(id="tenant-1", label="Tenant One", subkey="old-sub", quota=10, expires_at="2025-12-31", key_id="old-key")
...
def test_admin_revoke_endpoint(monkeypatch, tmp_path):
    ...
    Tenant = broker_app.Tenant  # type: ignore[attr-defined]
    tenant = Tenant(id="tenant-2", label="Tenant Two", subkey="sub", quota=5, key_id="key-to-revoke")
