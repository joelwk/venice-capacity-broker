...
def test_settlement_preview_with_slippage_and_pool_take(
    mock_dex_aggregator, mock_market_data, mock_etherscan_data, monkeypatch
):
    ...
    with (
        patch(
            "libs.dex.providers.build_aggregator_from_env",
            return_value=mock_dex_aggregator,
        ),
        patch(
        ...
        patch(
            "services.marketdata.etherscan_verify.get_cached_pair_info_for_tokens",
        ...
        app_module = importlib.import_module("apps.broker_api.app")
...
def test_settlement_preview_fallback_with_risk_hints(
    mock_market_data, mock_etherscan_data, monkeypatch
):
    ...
    with (
        patch("libs.dex.providers.build_aggregator_from_env", return_value=mock_agg),
        patch(
            "services.marketdata.provider.MarketDataProvider",
            return_value=mock_market_data,
        ),
...
def test_settlement_preview_exceeds_slippage_cap(mock_market_data, monkeypatch):
    ...
    with (
        ...
        response = client.get(
            "/v1/settlement/quote",
            params={"fromToken": "0xDIEM", "toAsset": "USDC", "amountOut": 1000000},
        )
...
def test_settlement_preview_exceeds_pool_take_cap(
    mock_market_data, mock_etherscan_data, monkeypatch
):
