...
_SKIP_REDIS_TESTS = os.getenv("SKIP_REDIS_TESTS", "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def test_rate_limit_enforced(monkeypatch, tmp_path):
    ...
    tenant = Tenant(id="t1", label="T1", subkey="sub-1", quota=TEST_TENANT_QUOTA)
...
def test_rate_limit_resets_without_redis(monkeypatch, tmp_path):
    ...
    # Insert tenant
    Tenant = broker_app.Tenant  # type: ignore[attr-defined]
    tenant = Tenant(id="t2", label="T2", subkey="sub-2", quota=TEST_TENANT_QUOTA)
    broker_app.store.upsert(tenant)
    ...
    r1 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "a1"}, json=payload
    )
    assert r1.status_code == 200
    r2 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "a2"}, json=payload
    )
    assert r2.status_code == 429
    # Wait for window to roll (monkeypatched to no-op but advances via fake_now)
    time.sleep(1.2)
    r3 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "a3"}, json=payload
    )
...
def test_rate_limit_with_redis_if_available(monkeypatch, tmp_path):
    ...
    # Insert tenant
    Tenant = broker_app.Tenant  # type: ignore[attr-defined]
    tenant = Tenant(id="t3", label="T3", subkey="sub-3", quota=TEST_TENANT_QUOTA)
    broker_app.store.upsert(tenant)
    ...
    r1 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "b1"}, json=payload
    )
    r2 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "b2"}, json=payload
    )
    assert r1.status_code == 200
    assert r2.status_code == 429
...
def test_no_rate_limit_when_disabled(monkeypatch, tmp_path):
    ...
    spec = importlib.util.spec_from_file_location(
        "broker_api_app_rl_off", str(app_path)
    )
    assert spec and spec.loader
    broker_app = importlib.util.module_from_spec(spec)
    # Register module in sys.modules before exec_module so module code can access sys.modules[__name__]
    ...
    Tenant = broker_app.Tenant  # type: ignore[attr-defined]
    tenant = Tenant(
        id="t_off", label="T_off", subkey="sub-off", quota=TEST_TENANT_QUOTA
    )
    broker_app.store.upsert(tenant)
    ...
    r1 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "z1"}, json=payload
    )
    r2 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "z2"}, json=payload
    )
    r3 = client.post(
        "/v1/chat", headers={**headers, "Idempotency-Key": "z3"}, json=payload
    )
    assert r1.status_code == 200
