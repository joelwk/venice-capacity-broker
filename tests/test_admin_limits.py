...
def _add_tenant(broker_app, tenant_id: str, subkey: str = "sub-1", label: str = "T1"):
    ...
    tenant = Tenant(id=tenant_id, label=label, subkey=subkey, quota=TEST_TENANT_QUOTA)
...
def test_admin_limits_post_valid_invalid_and_idempotent(tmp_path):
    ...
    r_post_404 = client.post(
        "/v1/tenants/nope/broker-limits", headers=headers, json=payload
    )
