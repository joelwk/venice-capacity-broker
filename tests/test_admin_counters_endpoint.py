...
def _make_fake_sql_stubs(rows):
    ...
    class Session:
        ...
        def exec(self, query):
            ...
            rows.sort(
                key=lambda r: r.bucket_start, reverse=not getattr(query, "_asc", False)
            )
            # Apply limit
...
def test_counters_validates_tenant_and_bucket_seconds(monkeypatch, tmp_path):
    ...
    try:
        ...
        r_bad_bs = client.get(
            "/v1/debug/counters",
            headers=headers,
            params={"tenant_id": "t1", "bucket_seconds": "abc"},
        )
...
def test_counters_filters_limit_and_asc(monkeypatch, tmp_path):
    ...
    rows = [
        ...
        SimpleNamespace(
            ...
            scope="chat",
            model=None,
            bucket_start=now - timedelta(minutes=4),
            bucket_seconds=60,
            count=2,
        ),
        SimpleNamespace(
