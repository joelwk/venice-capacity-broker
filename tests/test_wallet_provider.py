        ...
        for symbol in sorted(balances.keys()):
            ...
            if symbol == "ETH" and "wei" in entry:
                try:
                    ...
                    lines.append(f"  {symbol}: {human:.6f} ETH ({entry['wei']} wei)")
                except Exception:
            ...
            try:
                ...
                lines.append(
                    f"  {symbol}: {human:.6f} ({units} base units, "
                    f"decimals={decimals}, token={token_addr})"
                )
            except Exception:
                lines.append(
                    f"  {symbol}: {units} base units "
                    f"(decimals={decimals}, token={token_addr})"
                )
    else:
...
def test_adapter_normalizes_address_and_send(monkeypatch):
    ...
    web3_cls = web3_mod.Web3
    assert adapter.address == web3_cls.to_checksum_address(stub.get_address())
    tx_hash = adapter.send_transaction(
        to="0x4c4b1f1e1d1c1b1a191817161514131211100000", value=123
    )
    assert tx_hash == "0xtxhash"
    assert stub.sent is not None
    assert stub.sent["to"] == web3_cls.to_checksum_address(stub.get_address())
...
def test_adapter_sign_message_smart_wallet(monkeypatch):
    ...
    account_cls = acct_module.Account
    acct = account_cls.create()
    ...
    int(sig_hex, 16)
    assert len(sig_hex) == SIGNATURE_HEX_LENGTH


def test_transfer_from_cold_to_hot(monkeypatch):
    provider = _StubProvider()
    monkeypatch.setattr(
        "services.wallet.provider.get_agentkit_wallet", lambda: (provider, "eth")
    )

    captured = {}

    def fake_build(w3, from_addr, to, value):
    ...
    assert captured["build"] == (
        DummySigner.address,
        web3_cls.to_checksum_address(provider.get_address()),
        10,
    )
...
def test_sweep_requires_balance(monkeypatch):
    ...
    with pytest.raises(WalletError):
        sweep_profits_to_cold(
            100, cold_address="0xCc0d000000000000000000000000000000000001"
        )
