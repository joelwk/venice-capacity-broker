...
class _ErrorTradeExactOutProvider(DexProvider):
    ...
    def quote_exact_out(self, amount_out: int, path):  # noqa: ANN001
        return Quote(provider=self.name, amount_in=100, amount_out=amount_out, path=path)
...
def test_agg_trade_errors_exact_out_increments_metric():
    ...
    lines = [ln for ln in prom.splitlines() if ln.startswith("vvv_dex_agg_trade_errors_total{")]
