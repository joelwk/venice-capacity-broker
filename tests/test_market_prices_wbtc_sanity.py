...
def test_market_prices_clamps_wbtc(monkeypatch):
    ...
    monkeypatch.setattr(
        MarketDataProvider, "_check_wbtc_configuration", lambda self: None
    )
    monkeypatch.setattr(
        MarketDataProvider, "_warm_route_liquidity", lambda self, tokens: None
    )
    monkeypatch.setattr(
        MarketDataProvider, "_mid_price_from_reserves", lambda self, a, b: None
    )

    def fake_best_price(
        self: MarketDataProvider, route, amount_in_decimal: float = 1.0, **kwargs
    ):
    ...
    monkeypatch.setattr(
        MarketDataProvider,
        "_best_price_scan",
        lambda self, route, start=1.0, min_amount=1e-12, factor=10.0: None,
    )
    monkeypatch.setattr(
        MarketDataProvider,
        "_fetch_external_price",
        lambda self, symbol: 112000.0 if str(symbol).upper() == "WBTC" else None,
    )
