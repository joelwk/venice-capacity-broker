    ...
    def record_cycle(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if use_sql:
            try:
                ...
                _metrics_inc(
                    "sql_persist_error_total", labels={"entity": "agent_memory"}
                )
                if is_production():
        ...
        if not recorded:
            if is_production() or not env_flag("ALLOW_JSON_FALLBACK", False):
                ...
                raise RuntimeError(
                    "AgentMemory JSON fallback disabled; configure Postgres or set ALLOW_JSON_FALLBACK=1 in dev"
                )
            _metrics_inc(
                "fallback_json_store_total", labels={"component": "agent_memory"}
            )
        ...
        with Session(eng) as s:  # type: ignore[call-attr]
            ...
            try:
                if isinstance(payload, dict):
                    agent = str(
                        payload.get("agent") or payload.get("actor") or "system"
                    )
            except Exception:
                agent = "system"
            row = AgentMemory(
