...
try:
    from libs.agentkit_ext.agentkit_wallet import (
        get_agentkit_wallet as _agentkit_wallet_factory,
    )
...
# Placeholder values that should be treated as unset
_PLACEHOLDER_VALUES = {
    "set-in-secrets",
    "<set-in-secrets>",
    "",
...
def _is_placeholder(value: str | None) -> bool:
    """Check if a value is a placeholder that should be treated as unset."""
    if not value:
        return True
    normalized = str(value).strip().lower()
    return normalized in _PLACEHOLDER_VALUES or (
...
class AgentKitWalletAdapter:
    ...
    kind: str
    _cached_address: str | None = field(default=None, init=False, repr=False)
...
def transfer_from_cold_to_hot(
    amount_wei: int, cold_signer: TransactionSigner | None = None
) -> str:
    if amount_wei <= 0:
    ...
    w3 = get_web3()
    tx = build_eip1559_tx(
        w3, from_addr=signer.address, to=provider.address, value=int(amount_wei)
    )
    try:
...
def sweep_profits_to_cold(
    min_balance_wei: int,
    *,
    cold_address: str | None = None,
    gas_buffer_wei: int | None = None,
) -> str:
    ...
    if not dest or _is_placeholder(dest):
        raise WalletError(
            "Cold wallet address is required via argument or COLD_WALLET_ADDRESS (must be a valid Base address, not a placeholder)"
        )
    ...
    if buffer is None:
        ...
        if env_buffer:
            try:
                ...
                raise WalletError(
                    "WALLET_SWEEP_GAS_BUFFER_WEI must be an integer"
                ) from exc
        else:
...
def describe_treasury_portfolio(
    *,
    ...
    # Filter out placeholder values like "set-in-secrets"
    address_candidates = [
        wallet_address,
    ...
    if not resolved_address:
        errors.append(
            "treasury address unavailable (set TREASURY_ADDRESS or COLD_WALLET_ADDRESS to a valid Base address, not a placeholder)"
        )
        return snapshot
    ...
    defaults = {
        ...
        # sVVV (staked VVV) token - same address as the staking contract
        "SVVV": os.getenv("VVV_STAKING_ADDRESS"),
    }
