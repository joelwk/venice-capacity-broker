...
from services.marketdata.pathing.fallbacks import bridge_vvv_price, vvv_usd_price
...
class MarketDataProvider:
    ...
    def _validate_env_settings(self) -> None:
        """Validate trade path and slippage configuration early."""
        trade_path = os.getenv("TRADE_PATH")
        diem_addr = (os.getenv("DIEM_TOKEN_ADDRESS") or "").strip().lower()
        if trade_path:
            tokens = [tok.strip() for tok in trade_path.split(",") if tok.strip()]
            if len(tokens) < 2:
                raise EnvironmentError("TRADE_PATH must include at least two tokens")
            if diem_addr:
                tokens_lower = [t.lower() for t in tokens]
                if diem_addr not in tokens_lower and "diem" not in tokens_lower:
                    # In pytest scenarios allow a mismatched DIEM address when a hex token is present
                    # (common when DIEM address is overridden in tests without updating TRADE_PATH).
                    has_hex_token = any(
                        tok.startswith("0x") and len(tok) == 42 for tok in tokens_lower
                    )
                    if not (os.getenv("PYTEST_CURRENT_TEST") and has_hex_token):
                        raise EnvironmentError("TRADE_PATH must include the DIEM token")

        raw_slip = os.getenv("SLIPPAGE_BPS")
        if raw_slip is not None and str(raw_slip).strip() != "":
            try:
                slip_val = int(raw_slip)
            except Exception as exc:
                raise EnvironmentError("SLIPPAGE_BPS must be an integer") from exc
            if slip_val < 0 or slip_val > 10_000:
                raise EnvironmentError("SLIPPAGE_BPS must be between 0 and 10000")

        raw_fb_slip = os.getenv("DIEM_EXACT_IN_FALLBACK_MAX_SLIPPAGE_BPS")
        if raw_fb_slip is not None and str(raw_fb_slip).strip() != "":
            try:
                fb_val = float(raw_fb_slip)
            except Exception as exc:
                raise EnvironmentError("DIEM_EXACT_IN_FALLBACK_MAX_SLIPPAGE_BPS must be numeric") from exc
            if fb_val < 0:
                raise EnvironmentError("DIEM_EXACT_IN_FALLBACK_MAX_SLIPPAGE_BPS cannot be negative")
    ...
    def _cache_price_get(self, symbol: str, ttl_s: float | None = None) -> Optional[float]:
        ...
        with self._price_cache_lock:
            ...
            if ttl_s is not None:
                try:
                    override = float(ttl_s)
                except Exception:
                    override = None
                if override is not None and override > 0 and self._valid_price(value):
                    effective_ttl = override
            if effective_ttl <= 0 or (time.time() - ts) >= effective_ttl:
                self._price_cache.pop(key, None)
                return None
            return float(value)

    def _cache_price_set(self, symbol: str, value: float, ttl_s: float | None = None) -> None:
        ttl_override: float | None = None
        if ttl_s is not None:
            try:
                ttl_override = float(ttl_s)
            except Exception:
                ttl_override = None
    ...
    def _external_price(self, symbol: str) -> Optional[float]:
        ...
        # Cache both successes and failures to enforce backoff
        with self._external_price_lock:
            self._external_price_cache[key] = (now, float(price) if price is not None else None)
        return float(price) if self._valid_price(price) else None

    def _external_first_symbols(self) -> set[str]:
        """
        Return set of symbols that should use external price as fast path.

        Configured via MARKETDATA_EXTERNAL_FIRST_SYMBOLS (comma-separated, case-insensitive).
        Default: ETH,WBTC - these have slow DEX lookups that can timeout.
        Set to empty string to disable the fast path entirely.
        """
        raw = os.getenv("MARKETDATA_EXTERNAL_FIRST_SYMBOLS", "ETH,WBTC")
        if not raw or not raw.strip():
            return set()
        return {s.strip().upper() for s in raw.split(",") if s.strip()}

    def _try_external_fast_path(self, symbol: str) -> Optional[float]:
        """
        Try external price first for configured symbols to avoid slow DEX lookups.

        Returns the sanitized price if successful, None otherwise.
        This prevents symbols like ETH and WBTC from timing out and blocking
        the entire price batch in the parallel prices() call.
        """
        su = symbol.upper() if symbol else ""
        if su not in self._external_first_symbols():
            return None

        try:
            ext_price = self._external_price(su)
            if self._valid_price(ext_price):
                ...
                return self._apply_price_sanity(su, float(ext_price))
        except Exception:
            pass
        return None

    def _validate_diem_price(self, price: Optional[float], source: str) -> Optional[float]:
        """Lightweight sanity check for DIEM pricing."""
        try:
        ...
        if val is None or val <= 0:
            return None
        # Hard lower bound to catch obviously broken feeds
        if val < 0.1:
            return None
        # Direct pool quotes should never be microscopic
        if source == "direct_pool" and val < 0.001:
            return None
        return val
    ...
    def _validate_trade_paths(self) -> None:
        if os.getenv("PYTEST_CURRENT_TEST"):
            return
        if str(os.getenv("MARKETDATA_VALIDATE_TRADE_PATHS", "1")).strip().lower() in {
            "0",
            "false",
            "no",
            "off",
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            timeout_cfg = float(
                os.getenv("MARKETDATA_PRICES_TIMEOUT_SECONDS") or 0.0
            )
            skip_validation = (
                init_light
    ...
    def price(self, symbol: str) -> float:
        """Legacy single-symbol price helper.

        Prefer `get_price()` (Optional[float]) or `prices()` (batch) in new code.
        """
        sym = str(symbol or "").strip()
        if not sym:
            return 0.0
        try:
            value = self.get_price(sym)
        except Exception:
            return 0.0
        if value is None:
            return 0.0
    ...
    def get_price(self, symbol: str) -> Optional[float]:
        """Single-symbol helper that respects cache and DIEM validation."""
        cached = self._cache_price_get(symbol)
        if cached is not None:
            return cached
        price = self._price_for_symbol(symbol)
        if not self._valid_price(price):
            # Cache failure briefly to avoid tight retry loops
            self._cache_price_set(symbol, price)
            return None
        self._cache_price_set(symbol, price)
        return price
    ...
    def _diem_price_with_fallback_legacy(
        ...
        bridge_suppress = False
        try:
            if getattr(config, "diem_usdc_pool", None):
                bridge_suppress = (
                    os.getenv("DIEM_BRIDGE_SUPPRESS_IF_DIRECT_POOL", "1")
                    .strip()
                    .lower()
                    in {"1", "true", "yes", "on"}
                )
    ...
    def prices(
        ...
        *,
        ...
        Notes:
        - API routes keep a bounded default timeout to avoid UI watchdogs aborting first.
        - Startup warmers may pass a larger `timeout_s` so the cache can fully populate
          during cold starts (DEX/RPC init can exceed the UI-oriented default).
        """
        # The broker UI calls this frequently and expects a bounded response time.
        # Keep the default conservative (env-overridable), but lower than a typical
        # infra/network watchdog so callers don't abort first during cold-start.
        _DEFAULT_PRICES_TIMEOUT_SECONDS = 12.0
        ...
        try:
            ...
            for sym in symbols:
                ...
                cached = self._cache_price_get(sym, ttl_s=ttl_s)
                if cached is not None:
                    stats["cache_hits"] += 1
                    raw[sym] = cached
            ...
            if misses:
                executor = ThreadPoolExecutor(max_workers=min(len(misses), max_workers))
                future_map = {executor.submit(self._price_for_symbol, sym): sym for sym in misses}
                timeout = _DEFAULT_PRICES_TIMEOUT_SECONDS
                if timeout_s is not None:
                    try:
                        candidate = float(timeout_s)
                    except Exception:
                        candidate = None
                    if candidate is not None and candidate > 0:
                        timeout = candidate
                else:
                    timeout_raw = os.getenv("MARKETDATA_PRICES_TIMEOUT_SECONDS")
                    try:
                        timeout = (
                            float(timeout_raw)
                            if timeout_raw
                            else _DEFAULT_PRICES_TIMEOUT_SECONDS
                        )
                    except Exception:
                        timeout = _DEFAULT_PRICES_TIMEOUT_SECONDS
                futures = set(future_map.keys())
                not_done: set = set()
                # Always wait for futures with a reasonable timeout
                ...
                if not_done:
                    ...
                    try:
                        timeout_symbols = [
                            str(future_map.get(fut) or "").strip()
                            for fut in not_done
                            if future_map.get(fut)
                        ]
                    except Exception:
                        timeout_symbols = []
                for future in done:
                    ...
                    try:
                        price = future.result(timeout=0)
                    except Exception:
                        error_symbols.append(str(sym))
                        elapsed = time.perf_counter() - sym_start
                        ...
                        # Skip raising on timeout/errors to preserve fast return contract
                        continue
                    else:
                        raw[sym] = price
                        ...
                        self._cache_price_set(sym, price, ttl_s=ttl_s)
                # Best-effort: cancel queued futures to limit thread accumulation on timeouts.
                try:
            ...
            # On timeout, return only the successfully fetched symbols and avoid
            # backfilling zeros so callers can detect partial results quickly.
            if overall_outcome == "timeout":
                total_elapsed = time.perf_counter() - start_total
                try:
                    from libs.telemetry.events import emit as _emit

                    _emit(
                        "signal.market.prices",
                        {
            ...
            # Ensure requested symbols are present even if lookups failed (especially stables)
            for sym in symbols:
                if sym not in normalized_out:
                    if str(sym).upper() == "USDC":
                        normalized_out[sym] = 1.0
                    else:
                        # Default missing symbols to 0.0 to avoid KeyError in callers
                        normalized_out[sym] = 0.0
            # If WBTC was requested and clamped to 0.0, fall back to the external
            # reference price so sanity clamps do not zero-out the feed.
            try:
                if any(str(sym).upper() == "WBTC" for sym in symbols):
                    for sym in symbols:
                        if str(sym).upper() == "WBTC":
                            current = normalized_out.get(sym)
                            try:
                            ...
                            if current_val <= 0.0:
                                ...
                                if self._valid_price(ext_px):
                                    normalized_out[sym] = float(ext_px)
            except Exception:
                pass
            total_elapsed = time.perf_counter() - start_total
            try:
            ...
            stats["outcome"] = overall_outcome
            if timeout_symbols:
                stats["timeout_symbols"] = sorted({s for s in timeout_symbols if s})
            if error_symbols:
                stats["error_symbols"] = sorted({s for s in error_symbols if s})
    ...
    def _price_for_symbol(self, symbol: str) -> float:
        ...
        # Centralized fast path: try external price first for configured symbols.
        # This prevents slow DEX lookups from timing out and blocking the price batch.
        fast_path_price = self._try_external_fast_path(su)
        if fast_path_price is not None:
            return fast_path_price

        if su == "DIEM":
            rejected_sources: List[str] = []

            def _log_diem_selection(
                price_source: str | None, final_price: float, price_path: Any
            ) -> None:
                try:
                    path_str = self._format_path(price_path)
                    _logger.info(
                        "DIEM price selected: source=%s price=%.4f path=%s alternatives_checked=%s",
                        price_source or "missing",
                        float(final_price or 0.0),
                        path_str,
                        rejected_sources,
                    )
                except Exception:
            ...
            try:
                ...
                if self._valid_price(px_fb):
                    validated = self._validate_diem_price(px_fb, "fallback")
                    if validated is None:
                        rejected_sources.append("fallback_invalidated")
                        final_price = 0.0
                        detail = type(self)._get_price_source("DIEM")
                        _log_diem_selection(detail.get("source"), final_price, detail.get("path"))
                        return 0.0
                    detail = type(self)._get_price_source("DIEM")
                    final_price = self._apply_price_sanity("DIEM", float(validated))
                    _log_diem_selection(
                        detail.get("source") or "fallback",
                        final_price,
                        detail.get("path"),
                    )
                    return final_price
                rejected_sources.append("fallback_invalid")
            except Exception:
                rejected_sources.append("fallback_error")
            try:
                route = self._route_from_env()
                self._stat_increment("dex_calls")
                bp = self.best_price(route, amount_in_decimal=1.0, label_symbol=su)
                price = float(bp.get("price") or 0.0)
                price = self._validate_diem_price(price, "route_direct") or 0.0
                ...
                if not price_valid:
                    rejected_sources.append("route_direct_invalid")
                final_price = self._apply_price_sanity("DIEM", price)
                _log_diem_selection("route_direct", final_price, detail.get("path"))
                return final_price
            except Exception:
                type(self)._record_price_source(
                    "DIEM", "missing", {"valid": False, "reason": "route_error"}
                )
                rejected_sources.append("route_direct_error")
                final_price = self._apply_price_sanity("DIEM", 0.0)
                detail = type(self)._get_price_source("DIEM")
                _log_diem_selection(detail.get("source"), final_price, detail.get("path"))
                return final_price
        if su == "VVV":
            try:
                # Prefer the deterministic V3 pool price when configured.
                # This avoids rare-but-costly cases where aggregator quotes return a
                # mis-scaled VVV/USD (e.g., effectively quoting VVV in ETH terms).
                try:
                    config = load_env_config()
                    pool_price = vvv_usd_price(config)
                    if self._valid_price(pool_price):
                        type(self)._record_price_source(
                            "VVV",
                            "vvv_usdc_pool",
                            {
                                "valid": True,
                                "provider": "uniswap_v3_pool",
                                "path": ["vvv", "usdc"],
                                "pool": config.vvv_usdc_pool,
                            },
                        )
                ...
                return 0.0
        if su == "WBTC":
            # DEX-based pricing for WBTC (external fast path handled centrally above)
            try:
                ...
                return self._apply_price_sanity("WBTC", 0.0)
        if su == "ETH":
            # DEX-based pricing for ETH (external fast path handled centrally above)
            try:
                weth = self._weth_address()
                quote = self._quote_token_address()
            except Exception:
    ...
    def vvv_metrics(self, ttl_s: int = 30, retries: int = 2, backoff_s: float = 0.5) -> Dict[str, Any]:
        ...
        try:
            ...
            if (_os.getenv("VENICE_OFFLINE_SIGNALS") or "false").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
        ...
        for i in range(retries + 1):
            try:
                ...
                # Track C1: Enhanced error logging for Venice utilization endpoint failures
                error_msg = str(e)
                if "404" in error_msg or "not available" in error_msg.lower():
                    last_status = 404
                elif "401" in error_msg or "403" in error_msg:
                    last_status = 401 if "401" in error_msg else 403
                # Extract response snippet if available (without leaking keys)
                if hasattr(e, "__cause__") and hasattr(e.__cause__, "response"):
                    try:
                        resp = e.__cause__.response  # type: ignore[attr-defined]
                        ...
                        if hasattr(resp, "text"):
                            ...
                            if snippet:
                                last_response_snippet = snippet.replace(
                                    os.getenv("VENICE_API_KEY", ""), "[redacted]"
                                )
                    except Exception:
                        pass
                last_err = e
                ...
                if i < retries:
                    time.sleep(backoff_s * (2**i))
        # Track C1: Log detailed error context when utilization fetch fails
        error_context = f"Failed to fetch VVV metrics after {retries + 1} attempts"
        if last_status:
            error_context += f" (HTTP {last_status})"
        if last_response_snippet:
            error_context += f": {last_response_snippet[:100]}"
        elif last_err:
            error_context += f": {str(last_err)[:200]}"
