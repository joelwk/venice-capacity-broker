...
def _cache_get_path(token: str, quote: str) -> RoutePlan | None:
    try:
...
class TokenMetrics:
    ...
    supply_circulating: int | None
    holders: int | None
    transfers_24h: int | None
    marketcap_usd: float | None
    max_total_supply: int | None
    raw: dict[str, object]
...
class BaseScanClient:
    def __init__(
        self,
        api_key: str,
        base_url: str = DEFAULT_ETHERSCAN_V2_URL,
    ...
    def token_info(self, address: str) -> dict | None:
        try:
            j = self._get(
            ...
            if isinstance(res, dict):
                ti = (
                    res.get("tokenInfo")
                    if isinstance(res.get("tokenInfo"), dict)
                    else None
                )
                if ti:
                    out = dict(ti)
    ...
    def total_supply(self, address: str) -> int | None:
        try:
            j = self._get(
    ...
    def block_number_by_time(self, ts_epoch: int) -> int | None:
        try:
            j = self._get(
                {
                    "module": "block",
    ...
    def recent_transfers_count(
        self, address: str, minutes: int = 1440, max_events: int = 1000
    ) -> int | None:
...
def _log_missing_key_help() -> None:
    ...
    logger.info(
        "export BASE_RPC_URL='https://base.publicnode.com'  # For metadata lookups"
    )
    logger.info(
        "export QUOTE_TOKEN_ADDRESS='<quote-token-address>'  # e.g., USDC on Base mainnet"
    )


def _read_secret_candidate(path: str | Path) -> str | None:
    try:
        candidate = Path(path)
...
def _effective_quote_token_address() -> str | None:
    ...
    try:
        chain_id = int(
            os.getenv("ETHERSCAN_CHAIN_ID") or os.getenv("BASE_CHAIN_ID") or 8453
        )
    except Exception:
        chain_id = 8453
    ...
    if default_qt:
        _debug(
            f"QUOTE_TOKEN_ADDRESS not set; using default for chain {chain_id}: {default_qt}"
        )
    return default_qt


def _price_via_dex(address: str) -> float | None:
    ...
    try:
        ...
        tried: set[tuple[tuple[str, ...], tuple[int | None, ...]]] = set()
        ...
        def _route_from_tokens(
            tokens: list[str], fees: list[int | None] | None = None
        ) -> RoutePlan | None:
            try:
                return make_route(tokens, fees)
            except Exception as exc:
                _debug(
                    f"Unable to build route for {tokens} fees={fees}: {type(exc).__name__}: {exc}"
                )
                return None

        def _try_route(route: RoutePlan, label: str) -> tuple[float, str] | None:
            ...
            try:
                ...
                _debug(
                    f"{label} route success via {provider_val}: {_format_price(price_val)}"
                )
        ...
        if price is None:
            for route in candidate_routes:
                ...
                if tokens and tokens[0].lower() != address.lower():
                    continue
                label = (
                    "configured" if _route_key(route) in manual_keys else "candidate"
                )
        ...
        if price is None:
            direct_fees: list[int | None] = [None, 500, 1000, 3000, 10000]
            for fee in direct_fees:
                route = _route_from_tokens(
                    [address, quote], [fee] if fee is not None else None
                )
                if not route:
                    continue
        ...
        # Try bridge token (e.g., WETH) - skip for DIEM if canonical WETH routes disabled
        if price is None:
            # Check if this is DIEM and canonical WETH routes are disabled
            diem_addr = (os.getenv("DIEM_TOKEN_ADDRESS") or "").strip().lower()
            disable_weth_for_diem = _truthy_env("DIEM_DISABLE_CANONICAL_WETH")
            is_diem = diem_addr and address.lower() == diem_addr
            ...
            # Skip WETH bridge for DIEM when disabled - this avoids noisy quote failures
            # on routes without liquidity (DIEM→WETH pools are typically empty)
            weth_addr = DEFAULT_BRIDGE_TOKEN_BY_CHAIN.get(8453, "").lower()
            skip_bridge = (
                is_diem
                and disable_weth_for_diem
                and bridge
                and bridge.lower() == weth_addr
            )

            if (
                bridge
                and bridge.lower() not in {address.lower(), quote.lower()}
                and not skip_bridge
            ):
                for fee_in in (500, 1000, 3000):
                    for fee_out in (500, 1000, 3000):
                        route = _route_from_tokens(
                            [address, bridge, quote], [fee_in, fee_out]
                        )
                        if not route:
                            continue
                ...
                _debug(
                    "Skipping WETH bridge route for DIEM (DIEM_DISABLE_CANONICAL_WETH=1)"
                )

        # Try VVV as alternate intermediate hop
        if price is None:
            vvv_addr = os.getenv("VVV_TOKEN_ADDRESS") or next(
                (
                    addr
            ...
            if vvv_addr and vvv_addr.lower() not in {address.lower(), quote.lower()}:
                ...
                # Skip extended VVV→WETH→USDC path for DIEM when canonical WETH disabled
                if price is None and not (is_diem and disable_weth_for_diem):
                    try:
                        bridge_env = os.getenv("DEX_BRIDGE_TOKEN_ADDRESS")
                        try:
                            chain_id = int(
                    ...
                    if bridge and bridge.lower() not in {
                        address.lower(),
                        vvv_addr.lower(),
                        quote.lower(),
                    }:
...
def collect_token_metrics(address: str, client: BaseScanClient) -> TokenMetrics:
    ...
    _dbg_sources["metadata"] = (
        "web3" if sym2 or dec2 is not None else ("known" if symbol else "unknown")
    )

    # Supply and holders
    total_supply = client.total_supply(address)
    _dbg_sources["supply_total"] = (
        "stats.tokensupply" if total_supply is not None else "none"
    )
    ...
    try:
        ...
        _max_events = 200
    transfers_24h = client.recent_transfers_count(
        address, minutes=1440, max_events=_max_events
    )
    _dbg_sources["transfers_24h"] = (
        "logs.getLogs" if isinstance(transfers_24h, int) else "none"
    )
    ...
    if price_usd is None:
        ...
        try:
            max_price = float(os.getenv("TOKEN_WATCH_MAX_PRICE", "1000000"))
        except Exception:
            max_price = 1_000_000.0
        if px is not None and px > max_price:
            logger.warning(
                f"price sanity: {symbol or address[:8]} price={px:.2f} exceeds max={max_price}, clamping to None"
            )
            px = None
        price_usd = px
        if px is not None:
    ...
    try:
        if (
            ...
            and (supply_circ or total_supply)
            and decimals is not None
        ):
            base_supply = supply_circ if supply_circ is not None else total_supply
            if base_supply is not None:
                supply_tokens = float(base_supply) / float(10**decimals)
                marketcap_usd = float(price_usd) * supply_tokens
...
def persist_metrics(m: TokenMetrics) -> None:
    ...
    if _prod_like:
        ...
        _create_all = (
            ...
            and _create_all_env.strip().lower() in {"1", "true", "yes", "on"}
        )
        if not _create_all and _create_all_env is None:
            # Inform if not explicitly set in production
            import logging

            _logger = logging.getLogger("marketdata.token_watcher")
            _logger.info(
                "SQL_CREATE_ALL_ON_START not set in production-like environment; using migrations. Set SQL_CREATE_ALL_ON_START=true to enable auto-create"
            )
    else:
...
def run_watch_loop() -> None:
    ...
    allow_no_key = _truthy_env("AUTOSTART_TOKEN_WATCHER_ALLOW_NO_KEY") or _truthy_env(
        "TOKEN_WATCH_ALLOW_NO_KEY"
    )
    idle_logged = False
    ...
    base_url = (
        os.getenv("ETHERSCAN_API_URL")
        or os.getenv("BASESCAN_API_URL")
        or DEFAULT_ETHERSCAN_V2_URL
    )
    try:
        chain_id = int(
            os.getenv("ETHERSCAN_CHAIN_ID") or os.getenv("BASE_CHAIN_ID") or 8453
    ...
    _debug(
        f"  - UNISWAP_V2_ROUTER_ADDRESS: {os.getenv('UNISWAP_V2_ROUTER_ADDRESS', 'NOT SET')}"
    )
    _debug(
        f"  - AERODROME_ROUTER_ADDRESS: {os.getenv('AERODROME_ROUTER_ADDRESS', 'NOT SET')}"
    )
    ...
    if not addrs:
        raise RuntimeError(
            "No token addresses configured. Set TOKEN_WATCH_ADDRESSES or VVV_TOKEN_ADDRESS/DIEM_TOKEN_ADDRESS/USDC_ADDRESS"
        )
    ...
    logger.info(
        f"tracking {len(addrs)} token(s): {', '.join(addrs)}; interval={interval}s"
    )
    while True:
        for addr in addrs:
            try:
                ...
                logger.info(
                    f"{metrics.symbol or ''} {addr[:6]}.. price={_format_price(metrics.price_usd)} holders={metrics.holders} tx24h={metrics.transfers_24h}"
                )
            except Exception as e:
