...
from libs.dex.providers import Quote as DexQuote
...
def _approx_exec_price(
    amount_in: int, route: RoutePlan, fee_bps_per_hop: int = 30
) -> float | None:
    try:
        route.ensure_v2()
    except Exception:
...
def _quote_via_aggregator(
    ...
    try:
        quote = aggregator.best_quote(
            amount_in, route, allowed_providers=allowed_providers
        )
        if quote is None:
            return None
        return quote
        ...
        if elapsed > 1.5 and os.getenv("DIEM_DEBUG_ROUTES"):
            ...
            logger.debug(
                "aggregator quote latency %.3fs route=%s", elapsed, list(route.tokens)
            )
...
def _quote_to_dict(quote: DexQuote, route: RoutePlan) -> dict[str, Any]:
    ...
    if quote.amount_in > 0 and quote.amount_out > 0:
        ...
        if norm_in > 0:
            ...
            # Early validation: reject obviously wrong prices
            # Check for near-zero prices that suggest decimal/reserve inversion
            if price < 1e-6 and norm_in > 0 and norm_out > 0:
                # Possible inversion: try swapping decimals as sanity check
                # If swapping decimals gives reasonable price, log warning
                alt_price = norm_in / norm_out if norm_out > 0 else 0.0
                if alt_price > 0.01 and alt_price < 1e6:
                    # Likely inversion detected
                    import os

                    if os.getenv("MARKETDATA_DEBUG_SANITY") or os.getenv(
                        "DIEM_DEBUG_ROUTES"
                    ):
                        from libs.telemetry.logger import get_logger
                        ...
                        logger = get_logger("marketdata.pathing")
                        logger.warning(
                            "price inversion detected: price=%.6e alt=%.6f tokens=%s->%s dec_in=%d dec_out=%d amount_in=%d amount_out=%d",
                            price,
                            alt_price,
                            tokens[0][:10],
                            tokens[-1][:10],
                    ...
                    # Don't return inverted price - return 0 to trigger fallback
                    price = 0.0
    payload: dict[str, Any] = {
        ...
        "price": price,
        "path": list(
            getattr(quote, "path", quote.route.tokens if quote.route else tokens)
        ),
    }
    try:
        composite_slip = getattr(quote, "_composite_slippage_bps", None)
        if composite_slip is None:
            composite_slip = getattr(quote, "total_slippage_bps", None)
        if composite_slip is not None:
            payload["slippage_bps"] = float(composite_slip)
            payload["composite_slippage_bps"] = float(composite_slip)
    except Exception:
        pass
    return payload
    ...
    if token0 and token1:
        if ain == token0 and aout == token1:
            return (
                int(reserves[0]),
                int(reserves[1]),
                int(reserves[2] if len(reserves) > 2 else 0),
                True,
            )
...
def _pool_take_bps(amount_in: int, reserve_in: int) -> float | None:
...
def _price_lookup(price_map: dict[str, float], addr: str) -> float | None:
    if not price_map:
        return None
    key = _normalize(addr)
    if not key:
...
def _composite_slippage_from_prices(
    quote_payload: dict[str, Any],
    price_map: dict[str, float],
    tokens: list[str],
) -> float | None:
    """Compute slippage against market price when price map is available."""
    if not tokens or len(tokens) < 2:
        return None
    exec_price = 0.0
    try:
        exec_price = float(quote_payload.get("price") or 0.0)
    except Exception:
        exec_price = 0.0
    if exec_price <= 0:
        return None
    ref_in = _price_lookup(price_map, tokens[0])
    ref_out = _price_lookup(price_map, tokens[-1])
    if ref_in is None or ref_out is None or ref_in <= 0 or ref_out <= 0:
        return None
    expected_price = ref_out / ref_in
    if expected_price <= 0:
        return None
    diff = (expected_price - exec_price) / expected_price * 10_000.0
    return max(0.0, diff)


def _is_composite(plan: RoutePlan) -> bool:
    try:
        from libs.dex import composite as _comp

        checker = getattr(_comp, "is_composite_route", None)
        if callable(checker):
            return bool(checker(plan))
    except Exception:
        return False
    return False
...
def enrich_route(
    ...
    *,
    ...
    price_map = {
        (_normalize(k) if isinstance(k, str) else k): float(v)
        for k, v in (price_map or {}).items()
        if v is not None
    }

    quote = _quote_via_aggregator(
        aggregator,
    ...
    if evaluation.quote:
        if evaluation.quote.get("slippage_bps") is None and _is_composite(plan):
            slip = _composite_slippage_from_prices(evaluation.quote, price_map, tokens)
            if slip is not None:
                evaluation.quote["slippage_bps"] = slip
        if evaluation.quote and evaluation.quote.get("slippage_bps") is None:
    ...
    for idx in range(len(tokens) - 1):
        ...
        running_amount = _uni_v2_out(
            running_amount, reserve_in, reserve_out, fee_bps=int(fee_bps)
        )
        if running_amount <= 0:
            break
        hop_inputs.append(running_amount)
    ...
    for idx in range(len(tokens) - 1):
        ...
        if not info:
            ...
            # ensure verify ran
            _ensure_route_verified(plan)
            info = get_cached_pair_info_for_tokens(
                token_in, token_out
            ) or get_cached_pair_info_for_tokens(token_out, token_in)

        telemetry = HopTelemetry(
            token_in=token_in, token_out=token_out, pool=None, status="missing"
        )
        if info:
            ...
            reserve_in, reserve_out, timestamp, mapped = _pool_direction(
            ...
            if reserve_in is not None and reserve_out is not None and mapped:
                ...
                try:
                    ...
                    if price_in is not None and price_in > 0:
                        telemetry.metrics["reserve_in_usd"] = (
                            reserve_in_tokens * price_in
                        )
                    price_out = _price_lookup(price_map, token_out)
                    if price_out is not None and price_out > 0:
                        telemetry.metrics["reserve_out_usd"] = (
                            reserve_out_tokens * price_out
                        )
                except Exception:
                    pass
                ...
                if timestamp:
                    telemetry.metrics["age_seconds"] = max(
                        0, int(time.time()) - int(timestamp)
                    )
            else:
