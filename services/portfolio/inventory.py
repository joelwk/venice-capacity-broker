...
class PortfolioInventory:
    ...
    def __init__(
        self,
        *,
        marketdata_provider: Optional[Any] = None,
    ...
    def get_usdc_balance(self, snapshot: Optional[PortfolioSnapshot] = None) -> float:
        """Get USDC balance in USD (1:1)."""
        if snapshot is None:
            snapshot = self.snapshot(include_eth=False)
        return snapshot.per_asset_usd.get("USDC", 0.0)

    def get_vvv_balance(self, snapshot: Optional[PortfolioSnapshot] = None) -> float:
        """Get VVV balance in USD."""
        if snapshot is None:
            snapshot = self.snapshot(include_eth=False)
        return snapshot.per_asset_usd.get("VVV", 0.0)

    def get_diem_balance(self, snapshot: Optional[PortfolioSnapshot] = None) -> float:
        """Get DIEM balance in USD."""
        if snapshot is None:
            snapshot = self.snapshot(include_eth=False)
        return snapshot.per_asset_usd.get("DIEM", 0.0)

    def get_eth_balance(self, snapshot: Optional[PortfolioSnapshot] = None) -> float:
