...
from libs.pricing.engine import MarketPricingEngine, StaticPricingEngine
...
_PRICE_ENGINE_ENV = "PRICE_ENGINE"
_PRICE_ENGINE_MARKET = "market"
_PRICE_ENGINE_STATIC = "static"
_PRICE_UNIT_ETH_ENV = "PRICE_UNIT_ETH_WEI"
_PRICE_UNIT_USDC_ENV = "PRICE_UNIT_USDC"
...
def _resolve_price_engine_mode() -> str:
    """Resolve pricing engine mode.

    Priority:
    - If PRICE_ENGINE is set to "market" or "static", honor it.
    - Otherwise, auto-select:
      - static when PRICE_UNIT_* is configured (deterministic unit pricing)
      - market when PRICE_UNIT_* is not configured (buyer flow expects DIEM-aware pricing)
    """

    raw = (os.getenv(_PRICE_ENGINE_ENV) or "").strip().lower()
    if raw in {_PRICE_ENGINE_MARKET, _PRICE_ENGINE_STATIC}:
        return raw

    unit_eth = _parse_int_env(_PRICE_UNIT_ETH_ENV, default=0)
    unit_usdc = _parse_int_env(_PRICE_UNIT_USDC_ENV, default=0)
    if unit_eth > 0 or unit_usdc > 0:
        return _PRICE_ENGINE_STATIC
    return _PRICE_ENGINE_MARKET


def parse_discount_fraction(value: str | None) -> float | None:
    if value is None:
        return None
    raw = str(value).strip()
    if not raw:
...
def _refresh_session_bindings() -> None:
    ...
    current_engine = (
        getattr(get_session.__globals__, "get", lambda *a, **k: None)("get_engine")
        if hasattr(get_session, "__globals__")
        else None
    )
...
class PricingService:
    def __init__(self) -> None:
        ...
        self._warmup_timeout = float(
            os.getenv("PRICING_WARMUP_TIMEOUT_SECONDS") or "10.0"
        )
        self._lock_timeout = float(os.getenv("PRICING_LOCK_TIMEOUT_SECONDS") or "5.0")
        self._warmup_retry_enabled = (
            os.getenv("PRICING_WARMUP_RETRY_ENABLED") or "true"
        ).strip().lower() in {"1", "true", "yes", "on"}

        engine_mode = _resolve_price_engine_mode()

        # Choose engine: market or static
        if engine_mode == _PRICE_ENGINE_MARKET:
            self.engine = MarketPricingEngine()
            try:
                from services.marketdata.provider import (
                    MarketDataProvider,
                )  # lazy import
                ...
                logger.warning(
                    "failed to initialise market data provider; quotes will warm on first request: %s",
                    exc,
                )
                self._marketdata_provider = None
            ...
            def _warm_marketdata() -> None:
                ...
                if provider is None:
                    ...
                    return
                attempt = 0
                retry_delay = float(os.getenv("PRICING_WARMUP_RETRY_SECONDS") or "5.0")
                max_attempts_env = os.getenv("PRICING_WARMUP_MAX_ATTEMPTS")
                try:
                    max_attempts = (
                        int(max_attempts_env)
                        if max_attempts_env not in (None, "")
                        else 0
                    )
                except Exception:
                    max_attempts = 0
                while not self._marketdata_ready.is_set():
                    attempt += 1
                    try:
                        with self._marketdata_lock:
                            provider.prices(self._marketdata_symbols)
                        self._marketdata_ready.set()
                        logger.info(
                            "market data warm-up completed successfully (attempt=%s)",
                            attempt,
                        )
                        return
                    except Exception as warm_exc:
                        logger.warning(
                            "market data warm-up attempt %s failed: %s",
                            attempt,
                            warm_exc,
                        )
                        if max_attempts and attempt >= max_attempts:
                            break
                        sleep_seconds = max(retry_delay, 1.0)
                        try:
                            time.sleep(min(sleep_seconds * attempt, 30.0))
                        except Exception:
                            break
                if not self._marketdata_ready.is_set():
                    logger.error(
                        "market data warm-up failed after %s attempt(s); quotes remain gated",
                        attempt,
                    )

            Thread(
                target=_warm_marketdata, name="pricing-marketdata-warm", daemon=True
            ).start()
        else:
        ...
        self._quote_model: type[Any] | None = None
    ...
    @staticmethod
    def _parse_discount_fraction(value: str | None) -> float | None:
    ...
    @staticmethod
    def _clean_prices(data: dict[str, float] | None) -> dict[str, float]:
        if not data:
            return {}
        out: dict[str, float] = {}
    ...
    def _resolve_asset_usd(
        self, asset_u: str, market_prices: dict[str, float]
    ) -> float:
        ...
        try:
            if isinstance(self.engine, MarketPricingEngine):
                try:
                    ...
                    symbols = list(self._marketdata_symbols) or [
                        "DIEM",
                        "ETH",
                        "USDC",
                        "WBTC",
                    ...
                    warmup_ready = self._marketdata_ready.wait(
                        timeout=self._warmup_timeout
                    )
                    if not warmup_ready or not self._marketdata_ready.is_set():
                        logger.info(
                            "market data warm-up still in progress after %.1f seconds",
                            self._warmup_timeout,
                        )
                        raise RuntimeError(
                            "market data warming up; please retry shortly"
                        )
                    ...
                    while (
                        not lock_acquired
                        and (time.perf_counter() - start_lock_wait) < self._lock_timeout
                    ):
                    ...
                    if not lock_acquired:
                        logger.warning(
                            "market data lock acquisition timeout after %s seconds; proceeding without lock synchronization",
                            self._lock_timeout,
                        )
                        try:
                            prefetched = mdp.prices(symbols) or {}
                        except Exception as no_lock_exc:
                            logger.error(
                                "market data fetch failed without lock: %s", no_lock_exc
                            )
                            prefetched = {}
                    else:
                        try:
                    ...
                    prefetched_prices = (
                        dict(prefetched) if isinstance(prefetched, dict) else {}
                    )
                    ...
                    if hasattr(mdp, "price_health"):
                        try:
                            price_health_candidate = mdp.price_health(
                                "DIEM", max_age=180.0
                            )
                        except Exception:
                            price_health_candidate = None
                    if (
                        isinstance(price_health_candidate, dict)
                        and price_health_candidate
                    ):
                        ...
                        source_ok = any(
                            source_label_norm.startswith(prefix)
                ...
                if isinstance(self.engine, MarketPricingEngine):
                    ...
                    if not diem_ok:
                        ...
                        try:
                            ...
                            if (
                                float(pm2.get("DIEM") or 0.0) > 0
                                or float(base_usd or 0.0) > 0
                            ):
                    ...
                    if not diem_ok:
                        raise ValueError(
                            "DIEM pricing unavailable; check on-chain pools or set bridge addresses"
                        )
                    ...
                    def _resolve_positive_price(symbols: list[str]) -> float:
                        ...
                        if isinstance(fallback_map, dict):
                            for key, value in fallback_map.items():
                                if key not in price_map or price_map.get(key) in (
                                    None,
                                    0,
                                    0.0,
                                ):
                    ...
                    if asset_u in {"ETH", "WETH"}:
                        ...
                        if not (eth_usd and eth_usd > 0):
                            raise ValueError(
                                "ETH pricing unavailable for budget sizing"
                            )
                        ...
                        if not (wbtc_usd and wbtc_usd > 0):
                            raise ValueError(
                                "WBTC pricing unavailable for budget sizing"
                            )
                ...
                try:
                    ...
                    if market_data_duration > 0:
                        _metrics_inc(
                            ...
                            labels={
                                "bucket": _latency_bucket(market_data_duration / 1000.0)
                            },
                        )
                    _metrics_inc(
    ...
    def _persist_quote_from_payload(self, payload: dict[str, Any]) -> None:
        ...
        with next(get_session()) as session:  # type: ignore[call-arg]
            try:
                ...
                raise RuntimeError(
                    "sqlmodel unavailable for quote persistence"
                ) from exc
