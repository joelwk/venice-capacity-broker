    ...
    *,
    ...
    try:
        slippage = (
            ...
            else int(os.getenv("RISK_MAX_SLIPPAGE_BPS", "50"))
        )
        min_usd = (
            min_stake_usd
            if min_stake_usd is not None
            else float(os.getenv("STAKEMASTER_MIN_STAKE_USD", "1.0"))
        )
        ...
        if not quote_token or not vvv_token:
            ...
            result["errors"].append(
                "QUOTE_TOKEN_ADDRESS and VVV_TOKEN_ADDRESS must be set"
            )
            return result
        ...
        if amount_usdc < min_usd:
            result["status"] = "skipped"
            result["errors"].append(
                f"amount {amount_usdc:.2f} USD below minimum {min_usd:.2f} USD"
            )
            return result
        ...
        if stake_master and hasattr(stake_master, "stake_vvv"):
            stake_result = stake_master.stake_vvv(
                int(quote.amount_out), reason="profit_recycling"
            )
