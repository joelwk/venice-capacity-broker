...
2. Open Secrets and add required env vars (see `.env.example`). Minimal set:
   ...
   - KV store: `REPLIT_DB_URL` (or `KV_URL`) **and** `KV_API_TOKEN`; optional `KV_NAMESPACE=vvv`, `KV_PREFIX=vvv:`
 ...
 - DEX aggregator (for on-chain pricing in token watcher):
  ...
  - Memory & reflex (optional):
    - `AGENT_MEMORY_PATH` for cycle logs.
    - `REFLECTION_VOL_BPS_THRESHOLD`, `REFLECTION_HOLD_STREAK` tune critiques.
    - `REFLEX_MAX_VOL_BPS`, `REFLEX_MAX_UTILIZATION`, `REFLEX_MAX_PRICE_DRAWDOWN`, `REFLEX_APPLY_DRY_RUN`, `REFLEX_REQUIRE_ACTIVE_STAKE`.
...
- Enable features via env and restart broker:
  ...
  - Payments: `BASE_RPC_URL`, `TREASURY_ADDRESS`, `ACCEPT_ASSETS=ETH,USDC,WBTC`, `USDC_ADDRESS` for USDC, and `WBTC_TOKEN_ADDRESS`. Configure `WBTC_PRICE_PATH` (or `WBTC_TRADE_PATH`) before exposing WBTC so market data can stay glued to the deepest pools. The broker now prefers the WBTC->WETH->USDC bridge when no override exists and applies a Dexscreener-backed sanity clamp whenever the on-chain quote drifts more than 10%. Tweak the guardrails with `MARKETDATA_PRICE_SANITY_MAX_DRIFT` (decimal or percent form) and cache window `MARKETDATA_EXTERNAL_PRICE_TTL_SECONDS`.
