...
class StaticPricingEngine:
    ...
    def __init__(self) -> None:
        ...
        try:
            self._max_u = float(
                os.getenv("PRICE_ACCEPTED_MAX_UNITS", "1000000") or 1_000_000
            )
        except Exception:
    ...
    def price(self, units: float, asset: str) -> QuoteDraft:
        ...
        if unit_p <= 0:
            raise ValueError(
                "unit price not configured; set PRICE_UNIT_ETH_WEI or PRICE_UNIT_USDC"
            )
        try:
            uf = float(units)
        except Exception:
...
class MarketPricingEngine:
    ...
    def __init__(self) -> None:
        ...
        self._units_kind = (
            (
                os.getenv("PURCHASE_UNITS_KIND")
                or os.getenv("PRICE_UNITS_KIND")
                or "diem"
            )
            .strip()
            .lower()
        ...
        try:
            self._base_unit_usd_default = float(
                os.getenv("BASE_UNIT_USD", "0.1") or 0.1
            )
        except Exception:
        ...
        try:
            self._min_u = float(
                os.getenv(
                    "PRICE_ACCEPTED_MIN_UNITS", os.getenv("PRICE_MIN_UNITS", "0.01")
                )
                or 0.01
            )
        except Exception:
        ...
        self._prefetched_prices: dict[str, float] | None = None
        self._prefetched_provider: Any | None = None
    ...
    def price_from_budget(self, budget_amount: float, asset: str) -> QuoteDraft:
        ...
        if target_units < min_units:
            raise ValueError(
                f"budget must cover at least {min_units} DIEM (current min quote size)"
            )
    ...
    def _resolve_prices(self) -> tuple[float, dict[str, float]]:
        ...
        try:
            ...
            px_source = (
                ...
                else (mdp.prices(["DIEM", "ETH", "USDC", "WBTC"]) or {})
            )
    ...
    def price(self, units: float, asset: str) -> QuoteDraft:
        ...
        if asset_u == "USDC":
            ...
            if not (eth_usd and eth_usd > 0):
                raise ValueError(
                    "ETH pricing unavailable (missing ETH/USD from DEX); check QUOTE_TOKEN_ADDRESS and WETH route"
                )
            unit_p = int(round((float(base_unit_usd) / float(eth_usd)) * 1e18))
        elif asset_u == "WBTC":
            if not (wbtc_usd and wbtc_usd > 0):
                raise ValueError(
                    "WBTC pricing unavailable (missing WBTC/USD liquidity); configure WBTC_TOKEN_ADDRESS and trade paths"
                )
