...
- Buyer flow (flag-gated): enable `QUOTES_ENABLED=true` and `PURCHASES_ENABLED=true`, set `TREASURY_ADDRESS`, `ACCEPT_ASSETS=ETH,USDC`, and (for USDC) `USDC_ADDRESS`. The `/admin/buy.html` wizard renders the Market Snapshot sidebar, unlocks Step 2 only after a quote arrives (wallet connect + tx hash fields stay disabled until then), and shows Step 3 once verification returns the scoped key with its copy helper and expiry. Purchases also support SSE streaming of status.
