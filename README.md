...
- Buyer flow (flag-gated): enable `QUOTES_ENABLED=true` and `PURCHASES_ENABLED=true`, set `TREASURY_ADDRESS`, opt into WBTC by adding it to `ACCEPT_ASSETS` **and** configuring `WBTC_TOKEN_ADDRESS`, and keep `USDC_ADDRESS` set for USDC payments. The `/admin/buy.html` wizard renders the Market Snapshot sidebar, unlocks Step 2 only after a quote arrives (wallet connect + tx hash fields stay disabled until then), points out the guaranteed 5% discount (plus any utilization relief), and shows Step 3 once verification returns the scoped key with its copy helper and expiry. Purchases also support SSE streaming of status.
...
- Store secrets in a local copy of `docker/.env.local` (git-ignored) and keep the repository copy empty. Create the file locally from `docker/.env.local.example` and load values via your secrets manager when you run compose: `docker compose --env-file .env.docker --env-file docker/.env.local up`.

- Shared configuration lives in `.env`. Keep `.env.docker` free of secrets; copy `.env.docker.example` locally when you need non-sensitive overrides and leave the tracked stub as documentation only.
