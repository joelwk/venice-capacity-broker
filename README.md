...
Buyer Flow (flag‑gated):
  - Baseline: enable `QUOTES_ENABLED=true` and `PURCHASES_ENABLED=true`, set `TREASURY_ADDRESS`, `ACCEPT_ASSETS=ETH,USDC`, and (for USDC) `USDC_ADDRESS`. Open `/admin/buy.html` -> Step 1: request a quote (copy amount/address, watch the countdown) -> Step 2: confirm payment after the quote unlocks the wallet + tx inputs -> Step 3: verify and copy the issued key. Purchases also support SSE streaming of status.
