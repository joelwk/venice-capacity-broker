...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rotate TRADE_PATH entries after verifying liquidity."
    )
    parser.add_argument(
        "--env-file",
        default=".env",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Decimal amount used when quoting (default: 1.0)",
    )
    parser.add_argument(
        "--force", action="store_true", help="Swap regardless of primary health"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Do not modify the env file"
    )
    parser.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip creating a .bak backup when writing",
    )
    return parser.parse_args()


def _evaluate(
    provider: MarketDataProvider, spec: str | None, amount: float
...
def main() -> int:
    ...
    print(
        f"Primary : {primary_spec or '<unset>'}\n  status : {primary_status}\n  price  : {primary_price}"
    )
    print(
        f"Secondary : {secondary_spec or '<unset>'}\n  status   : {secondary_status}\n  price    : {secondary_price}"
    )

    should_swap = False
    if args.force or (primary_price is None and secondary_price is not None):
        should_swap = True
    ...
    _swap_env(
        ...
        primary_spec or "",
        secondary_spec or "",
        update_paths_json=update_paths_json,
