...
def probe(url: str) -> ProbeResult:
    ...
    height = None
    try:
        response = requests.post(url, data=json.dumps(JSON_PAYLOAD), headers={"Content-Type": "application/json"}, timeout=5)
    ...
    return ProbeResult(url=url, ok=ok, latency_ms=latency_ms, status_code=status_code, error=error, height=height)
