...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("--tenant-id", default=os.getenv("TEST_TENANT_ID", "t-test"), help="Tenant id (default: t-test)")
    p.add_argument("--label", default=os.getenv("TEST_TENANT_LABEL", "Test Tenant"), help="Tenant label")
    p.add_argument("--quota", type=int, default=None, help="Optional quota override (int)")
    p.add_argument("--expires-at", default=None, help="Optional ISO8601 expiry (e.g., 2025-12-31T23:59:00Z)")
    p.add_argument("--rotate", action="store_true", default=False, help="Rotate key if tenant exists")
    p.add_argument("--revoke-old", action="store_true", default=False, help="Revoke previous key on rotate")
    p.add_argument("--probe-chat", action="store_true", default=False, help="Send a hello message via admin act-as")
    ...
    if not (os.getenv("VENICE_PARENT_KEY") or os.getenv("VENICE_API_KEY")):
        print("WARNING: VENICE_PARENT_KEY or VENICE_API_KEY is not set — subkey issuance may fail", file=sys.stderr)
    ...
    if args.probe-chat:
        ...
        try:
            ...
            detail = cr.text
        print(json.dumps({"chat_status": cr.status_code, "ok": ok, "response": detail}, indent=2))
