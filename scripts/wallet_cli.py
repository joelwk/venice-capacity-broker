...
def _parse_wei(value: str) -> int:
    try:
        ...
        raise WalletError("value must be an integer (optionally prefixed with 0x)") from exc
...
def _hex_to_bytes(raw: str) -> bytes:
    ...
    except binascii.Error as exc:  # noqa: BLE001
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    tx_hash = provider.send_transaction(to=args.to, data=data, value=_parse_wei(args.value))
    print(tx_hash)
