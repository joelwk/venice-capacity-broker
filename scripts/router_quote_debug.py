...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect router quotes for configured trade path"
    )
    parser.add_argument(
        "--mode", choices=("v2", "v3"), default="v2", help="router flavor to query"
    )
    parser.add_argument(
        "--amount", type=int, help="amount-in in base units (defaults to 1 token)"
    )
    parser.add_argument(
        "--path", type=str, help="override route spec (defaults to TRADE_PATH)"
    )
    parser.add_argument(
        "--exact-out", dest="exact_out", type=int, help="quote exact-out requirement"
    )
...
def main() -> None:
    ...
    if any(h.fee is None for h in route.hops):
        if provider_default_fee is None:
            raise SystemExit(
                "route is missing fee tiers and UNISWAP_V3_DEFAULT_FEE is unset"
            )
        filled_route = route.with_default_fee(int(provider_default_fee))

    quoter_contract = get_contract(
        w3, Web3.to_checksum_address(quoter), "uniswap_v3_quoter.json"
    )
    try:
        out_data = quoter_contract.functions.quoteExactInput(
            filled_route.to_uniswap_v3_path_bytes(), amount
        ).call()
