...
def main():
    ...
    expected_prices = {"VVV": 2.63, "DIEM": 227.26, "ETH": 4401.82, "USDC": 1.0}
    ...
    print(
        f"      Ratio of ratios: {usdc_diem_ratio_given / usdc_diem_ratio_expected:,.2e}"
    )
    ...
    for token, given in given_prices.items():
        ...
        print(
            f"   {token}: {given:.2e} × {scaling_factor:.2e} = ${scaled:.2f} (expected ${expected:.2f}, error {error:.1f}%)"
        )
