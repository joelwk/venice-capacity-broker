...
bash scripts/print_env_diagnostics.sh docker_start_broker

if [ -f /.dockerenv ]; then
  log "Detected Docker environment (/.dockerenv present)"
else
  log "Not running inside Docker container"
fi

if [ -n "${REDIS_URL:-}" ]; then
  log "REDIS_URL is configured and will be used for durable KV"
elif [ -n "${KV_REDIS_URL:-}" ]; then
  log "KV_REDIS_URL is configured (fallback for Redis)"
else
  log "No Redis URL configured; fallback may rely on in-memory KV"
fi

log "ALLOW_INMEMORY_KV_FALLBACK=${ALLOW_INMEMORY_KV_FALLBACK:-<unset>}"
...
# Disable in-memory KV fallback when live mode is enabled
if [ "${AUTOSTART_ORCHESTRATOR_LIVE:-0}" = "1" ] || [ "${AUTOSTART_STAKEMASTER_LIVE:-0}" = "1" ]; then
  log "Live mode detected - disabling in-memory KV fallback"
  export ALLOW_INMEMORY_KV_FALLBACK=0
  
  # Validate durable KV store is available
  if [ -z "${REDIS_URL:-}" ] && [ -z "${REPLIT_DB_URL:-}" ] && [ -z "${KV_REDIS_URL:-}" ]; then
    log "ERROR: Live mode requires durable KV store (REDIS_URL, KV_REDIS_URL, or REPLIT_DB_URL)"
    exit 1
  fi
fi
